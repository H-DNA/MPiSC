...
#proof[
  ...
  - An enqueue that returns `true` will be matched if there are enough dequeues after that: Based on how @ltqueue-enqueue is defined, when an enqueue returns `true`, it has successfully executed `spsc_enqueue`. By @ltqueue-unmatched-enqueue-theorem, at some point, it would eventually be matched.
...
#proof[
  ...
  - If the queue is empty, a dequeue returns `false`. Suppose a dequeue $d$ such that any $e arrow.double$#sub($H'$)$d$ is all matched by some $d'$ and $d' arrow.double$#sub($H'$)$d$, we will prove that $d$ is unmatched. By @slotqueue-matching-dequeue-enqueue-theorem, $d$ can only match an enqueue $e_0$ that precedes or overlaps with $d$.
    ...
    - If $e_0$ overlaps with $d$, by our assumption, $d arrow.double$#sub($H'$)$e_0$ because if $e_0 arrow.double$#sub($H'$)$d$, $e_0$ is already matched by another $d'$. Then, we can only obtain this because $(4)$ applies, but then $d$ does not match $e_0$.
    Therefore, $d$ is unmatched.
  - A dequeue returns `true` and matches an enqueue when the queue is not empty: To put more precisely, for a dequeue $d$, if there exists a successful enqueue $e'$ such that $e' =>$#sub($H'$)$d$ and has not been matched by a dequeue $d'$ such that $d' =>$#sub($H'$)$e'$, then $d$ would match some $e$ and return `true`. This follows from @slotqueue-unmatched-enqueue-theorem.
  - An enqueue that returns `true` will be matched if there are enough dequeues after that: Based on how @slotqueue-enqueue is defined, when an enqueue returns `true`, it has successfully executed `spsc_enqueue`. By @slotqueue-unmatched-enqueue-theorem, at some point, it would eventually be matched.
