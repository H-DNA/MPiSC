...
template <typename data_t> class Spsc_2 {
  int _self_rank;
...
public:
  Spsc_2(MPI_Aint capacity, MPI_Aint dequeuer_rank, MPI_Comm comm,
  ...
  Spsc_2(const Spsc_2 &) = delete;
  Spsc_2 &operator=(const Spsc_2 &) = delete;
  Spsc_2 &operator=(Spsc_2 &&) = delete;
  ...
  bool enqueue(const data_t &data) {
    ...
    if (new_last - this->_first_buf[this->_self_rank] > this->_capacity) {
      aread_sync(&this->_first_buf[this->_self_rank], 0, this->_self_rank,
                 this->_first_win);
      if (new_last - this->_first_buf[this->_self_rank] > this->_capacity) {
        return false;
      }
