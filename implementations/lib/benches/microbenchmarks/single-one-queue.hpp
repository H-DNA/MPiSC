...
#include "../../lqueue/lqueue.hpp"
#include "../../ltqueue/eo-ltqueue.hpp"
...
#include "../../ltqueue/naive-ltqueue.hpp"
#include "../../mpqueue/mpqueue.hpp"
#include "../../slotqueue/hosted-slotqueue.hpp"
#include "../../slotqueue/slotqueue-node-2.hpp"
...
inline void
mpqueue_single_one_queue_microbenchmark(unsigned long long number_of_elements,
                                        int iterations = 10) {
  int size;
  int rank;
  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    if (rank == 0) {
      MPQueue<int> queue(0, MPI_COMM_WORLD);
  ...
  report_single_one_queue(
      "MPQueue", number_of_elements, iterations, total_microseconds,
...
inline void slotqueue_node_2_single_one_queue_microbenchmark(
    unsigned long long number_of_elements, int iterations = 10) {
  int size;
  int rank;
  MPI_Comm_size(MPI_COMM_WORLD, &size);
  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    if (rank == 0) {
      SlotNodeQueue_2<int> queue(elements_per_queue, 0, MPI_COMM_WORLD);
      MPI_Barrier(MPI_COMM_WORLD);
      auto t1 = std::chrono::high_resolution_clock::now();
      while (local_successful_dequeues < number_of_elements) {
        int output;
  ...
  report_single_one_queue(
      "Slotqueue Node 2", number_of_elements, iterations, total_microseconds,
      total_dequeues, total_successful_dequeues, total_dequeues_microseconds,
      total_enqueues, total_successful_enqueues, total_enqueues_microseconds,
      total_enqueues_latency_microseconds);
}
...
inline void naive_unbounded_ltqueue_single_one_queue_microbenchmark(
    unsigned long long number_of_elements, int iterations = 10) {
  int size;
  int rank;
  MPI_Comm_size(MPI_COMM_WORLD, &size);
...
inline void naive_ltqueue_single_one_queue_microbenchmark(
  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    if (rank == 0) {
      NaiveLTQueue<int> queue(elements_per_queue, 0, MPI_COMM_WORLD);
      MPI_Barrier(MPI_COMM_WORLD);
      auto t1 = std::chrono::high_resolution_clock::now();
      while (local_successful_dequeues < number_of_elements) {
        int output;
  ...
  report_single_one_queue(
      "Naive LTQueue", number_of_elements, iterations, total_microseconds,
      total_dequeues, total_successful_dequeues, total_dequeues_microseconds,
      total_enqueues, total_successful_enqueues, total_enqueues_microseconds,
      total_enqueues_latency_microseconds);
}

inline void eo_ltqueue_single_one_queue_microbenchmark(
    unsigned long long number_of_elements, int iterations = 10) {
  int size;
  int rank;
  MPI_Comm_size(MPI_COMM_WORLD, &size);
  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    if (rank == 0) {
      EoLTQueue<int> queue(elements_per_queue, 0, MPI_COMM_WORLD);
      MPI_Barrier(MPI_COMM_WORLD);
      auto t1 = std::chrono::high_resolution_clock::now();
      while (local_successful_dequeues < number_of_elements) {
        int output;
  ...
  report_single_one_queue(
      "Enqueue-optimized LTQueue", number_of_elements, iterations,
      total_microseconds, total_dequeues, total_successful_dequeues,
      total_dequeues_microseconds, total_enqueues, total_successful_enqueues,
      total_enqueues_microseconds, total_enqueues_latency_microseconds);
}
...
inline void
lqueue_single_one_queue_microbenchmark(unsigned long long number_of_elements,
                                       int iterations = 10) {
  int size;
  int rank;
  MPI_Comm_size(MPI_COMM_WORLD, &size);
  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    if (rank == 0) {
      LQueue<int> queue(number_of_elements, 0, MPI_COMM_WORLD);
      MPI_Barrier(MPI_COMM_WORLD);
      auto t1 = std::chrono::high_resolution_clock::now();
      while (local_successful_dequeues < number_of_elements) {
        std::vector<int> output;
  ...
  report_single_one_queue(
      "LQueue", number_of_elements, iterations, total_microseconds,
      total_dequeues, total_successful_dequeues, total_dequeues_microseconds,
      total_enqueues, total_successful_enqueues, total_enqueues_microseconds,
      total_enqueues_latency_microseconds);
}
